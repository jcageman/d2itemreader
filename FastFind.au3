...
global const $FFCurrentVersion="2"
...
; Proto C function: int WINAPI ColorPixelSearch(int &XRef, int &YRef, int ColorToFind, int NoSnapShot)
 Func FFPixelInRectangle($StartX, $StartY, $EndX, $EndY, $Color, $VerticalFirst = False)
 	;local $NoSnapShot = 2 ; Slot utilisé pour les captures d'écran (entre 0 et 3), choisi arbitrairement
	if Not SnapShotPreProcessor(0, 0, 0, 0, False, $FFLastSnap, -1) Then
		SetError(2)
		Return False
	EndIf
	Local $Result = DllCall($FFDllHandle, "int", "ColorPixelSearchRectangle", "int*", $StartX, "int*",$StartY, "int", $StartX, "int",$StartY, "int", $EndX, "int",$EndY, "int", $Color, "BOOLEAN", $VerticalFirst, "int", $FFLastSnap)
